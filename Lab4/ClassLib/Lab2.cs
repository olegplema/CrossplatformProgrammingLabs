...
public class Lab2
{
    public static void Execute(string inputPath, string outputPath)
    {
        try
        {
            ...
            WriteOutputFile(outputPath, result, minCost);
        }
        catch (Exception e)
        {
    ...
    private static (int m, int n, int[,] matrix) ParseInputFile(string inputPath)
    {
    ...
    private static void WriteOutputFile(string outputPath, int[] result, int total)
    {
        var resultString = String.Join(" ", result); 
        File.WriteAllText(outputPath, $"{resultString}{Environment.NewLine}{total}");
    }
